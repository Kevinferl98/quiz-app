...
def client_with_mocks():
    with patch("app.api.routes_ws.redis_client") as mock_redis, \
        ...
        yield client, mock_redis, mock_manager

def test_host_can_start_quiz(client_with_mocks):
    client, mock_redis, mock_manager = client_with_mocks

    mock_redis.get_room_meta.return_value = {"owner_id": "host_user"}

    with client.websocket_connect("/ws/rooms/room123/host_user") as websocket:
...
def test_player_cannot_start_quiz(client_with_mocks):
    ...
    with client.websocket_connect("/ws/rooms/room123/player_1") as websocket:
        websocket.send_json({"type": "start"})
...
def test_player_can_join(client_with_mocks):
    ...
    mock_redis.get_room_meta.return_value = {"owner_id": "host_user"}
    mock_redis.get_players.return_value = [{"name": "Mario"}]

    with client.websocket_connect("/ws/rooms/room123/player_456") as websocket:
        websocket.send_json({"type": "join", "name": "Mario"})
    ...
    assert "Mario" in broadcast_calls[0]["players"]

def test_host_join_is_ignored(client_with_mocks):
    client, mock_redis, mock_manager = client_with_mocks

    mock_redis.get_room_meta.return_value = {"owner_id": "host_user"}

    with client.websocket_connect("/ws/rooms/room123/host_user") as websocket:
        websocket.send_json({"type": "join", "name": "Host"})

    mock_redis.add_player.assert_not_called()
